...
begin
  ...
  where p.id = target_package_id and p.status = 'active';
  ...
  for target_course in
    ...
    where pc.package_id = target_package_id and c.status = 'published'
  loop
  ...
  where e.user_id = auth.uid() and e.package_id = target_package_id
