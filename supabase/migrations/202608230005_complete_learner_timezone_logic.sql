...
-- The old index grouped game completions by UTC date. Local-day uniqueness is
-- enforced by the transaction lock and the learner-local lookup below.
drop index if exists public.learning_activity_events_game_completion_daily_unique;
...
begin
  ...
  today := public.learner_local_date();
  -- ponytail: one lock per learner keeps this migration small; shard by course/game only if contention is measurable.
  ...
  where e.user_id = auth.uid()
    ...
    and public.learner_local_date(auth.uid(), e.occurred_at) = today
...
as $$
with today as (
  select public.learner_local_date() as local_day
), active_days as (
  select distinct public.learner_local_date(auth.uid(), occurred_at) as day
...
begin
  ...
  from public.learning_activity_events
  where user_id = auth.uid()
    and public.learner_local_date(auth.uid(), occurred_at) = today;

  while exists (
    select 1
    from public.learning_activity_events
    where user_id = auth.uid()
      and public.learner_local_date(auth.uid(), occurred_at) = cursor_day
  ) loop
    streak := streak + 1;
...
begin
  ...
  where (ls.in_app_notifications or ls.email_notifications or ls.push_notifications)
    ...
    and not exists (
      ...
      where n.user_id = ls.user_id
        ...
        and public.learner_local_date(ls.user_id, n.created_at) = public.learner_local_date(ls.user_id)
    );
  ...
  where n.notification_type = 'review_due'
    and public.learner_local_date(n.user_id, n.created_at) = public.learner_local_date(n.user_id)
