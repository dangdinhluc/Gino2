...
export interface CommunityLeaderboardEntry { userId: string; displayName: string; handle: string; weeklyXp: number; currentStreak: number }
export interface CommunityGroup { id: string; courseId: string | null; name: string; description: string; memberCount: number; joined: boolean }
export interface CommunityReportInput { targetType: 'profile' | 'post' | 'message'; targetId: string; reason: string }
...
const mapLeaderboard = (row: { user_id: string; display_name: string; handle: string; weekly_xp: number; current_streak: number }): CommunityLeaderboardEntry => ({ userId: row.user_id, displayName: row.display_name, handle: row.handle, weeklyXp: Number(row.weekly_xp), currentStreak: Number(row.current_streak) });
const mapGroup = (row: { id: string; course_id: string | null; name: string; description: string; member_count: number; joined: boolean }): CommunityGroup => ({ id: row.id, courseId: row.course_id, name: row.name, description: row.description, memberCount: Number(row.member_count), joined: row.joined });
...
export async function saveCommunityProfile(input: { handle: string; bio: string; isPublic: boolean }): Promise<CommunityMe> {
  ...
  if (error) throw new Error(error.message);
  if (!data?.[0]) throw new Error('Không lưu được hồ sơ cộng đồng.');
...
export async function followCommunityUser(userId: string): Promise<void> { const { error } = await requireSupabase().rpc('follow_community_user', { target_user_id: userId }); if (error) throw new Error(error.message); }
export async function unfollowCommunityUser(userId: string): Promise<void> { const { error } = await requireSupabase().rpc('unfollow_community_user', { target_user_id: userId }); if (error) throw new Error(error.message); }
export async function blockCommunityUser(userId: string): Promise<void> { const { error } = await requireSupabase().rpc('block_community_user', { target_user_id: userId }); if (error) throw new Error(error.message); }
export async function unblockCommunityUser(userId: string): Promise<void> { const { error } = await requireSupabase().rpc('unblock_community_user', { target_user_id: userId }); if (error) throw new Error(error.message); }
export async function reportCommunityContent(input: CommunityReportInput): Promise<void> { const { error } = await requireSupabase().rpc('report_community_content', { target_type: input.targetType, target_id: input.targetId, target_reason: input.reason.trim() }); if (error) throw new Error(error.message); }
export async function getCommunityFeed(): Promise<CommunityPost[]> { const { data, error } = await requireSupabase().rpc('get_community_feed', { target_limit: 50 }); if (error) throw new Error(error.message); return (data ?? []).map((row) => ({ postId: row.post_id, userId: row.user_id, displayName: row.display_name, handle: row.handle, body: row.body, postType: row.post_type, metadata: (row.metadata ?? {}) as Record<string, unknown>, createdAt: row.created_at })); }
export async function createCommunityPost(body: string): Promise<void> { const { error } = await requireSupabase().rpc('create_community_post', { target_body: body.trim() }); if (error) throw new Error(error.message); }
export async function createProgressPost(): Promise<{ dailyXp: number; currentStreak: number }> { const { data, error } = await requireSupabase().rpc('create_progress_post'); if (error) throw new Error(error.message); const row = data?.[0]; if (!row) throw new Error('Không chia sẻ được tiến độ.'); return { dailyXp: row.daily_xp, currentStreak: row.current_streak }; }
export async function getCommunityLeaderboard(): Promise<CommunityLeaderboardEntry[]> { const { data, error } = await requireSupabase().rpc('get_community_leaderboard', { target_limit: 20 }); if (error) throw new Error(error.message); return (data ?? []).map(mapLeaderboard); }
export async function listCommunityGroups(): Promise<CommunityGroup[]> { const { data, error } = await requireSupabase().rpc('list_community_groups', { target_limit: 30 }); if (error) throw new Error(error.message); return (data ?? []).map(mapGroup); }
