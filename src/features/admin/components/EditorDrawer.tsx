...
export function EditorDrawer({ open, title, description, onRequestClose, children, footer }: EditorDrawerProps) {
  ...
  useEffect(() => {
    if (open && !dialogRef.current?.open) openerRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  ...
  return (
    <dialog ref={dialogRef} aria-labelledby="admin-editor-title" aria-describedby={description ? 'admin-editor-description' : undefined} onClick={(event) => { if (event.target === dialogRef.current) onRequestClose(); }} className="m-0 h-dvh max-h-none w-full max-w-none border-0 bg-transparent p-0 backdrop:bg-[#172033]/35 lg:ml-auto lg:w-[min(42rem,100vw)]">
      <section className="flex h-full flex-col bg-[#FFFCF7] text-[#172033] shadow-[-20px_0_50px_rgba(23,32,51,0.18)]">
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[#E4D8C9] px-5 py-4">
          <div className="min-w-0"><h2 id="admin-editor-title" className="text-xl font-bold">{title}</h2>{description && <p id="admin-editor-description" className="mt-1 text-sm leading-6 text-[#5F6B7C]">{description}</p>}</div>
