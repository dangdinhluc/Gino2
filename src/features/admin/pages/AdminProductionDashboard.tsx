...
type ReviewQuestion = Awaited<ReturnType<typeof listAdminReviewQuestions>>[number];
...
type SectionId =
  ...
  | 'revisions'
  | 'activity';
type FormKind = 'text' | 'textarea' | 'number' | 'time' | 'select' | 'multi';
...
const PACKAGE_STATUS_OPTIONS = [{ value: 'draft', label: 'Nháp' }, { value: 'active', label: 'Đang mở' }, { value: 'archived', label: 'Lưu trữ' }];
const ALERT_STATUS_OPTIONS = [{ value: 'open', label: 'Đang mở' }, { value: 'resolved', label: 'Đã xử lý' }];
const PROMPT_STATUS_OPTIONS = [{ value: 'draft', label: 'Nháp' }, { value: 'active', label: 'Đang dùng' }, { value: 'archived', label: 'Lưu trữ' }];
const SITE_PAGE_STATUS_OPTIONS = [{ value: 'draft', label: 'Nháp' }, { value: 'published', label: 'Đã công bố' }, { value: 'archived', label: 'Lưu trữ' }];

function isContentRole(role: AdminStaffRole): boolean {
  return role === 'owner' || role === 'content_editor';
}

function isLearnerRole(role: AdminStaffRole): boolean {
  return role === 'owner' || role === 'instructor_support';
}

function isAnalyticsRole(role: AdminStaffRole): boolean {
  return role === 'owner' || role === 'analyst';
}

function isAnnouncementRole(role: AdminStaffRole): boolean {
  return role === 'owner' || role === 'instructor_support';
}

function sectionsFor(role: AdminStaffRole): Array<{ id: SectionId; label: string; icon: typeof Layers3 }> {
...
function rowsFor(data: ProductionData, section: SectionId): Row[] {
  switch (section) {
...
function draftFromRow(section: SectionId, row: Row | null, data: ProductionData): Record<string, string> {
  if (!row) {
...
export default function AdminProductionDashboard() {
  ...
  const [data, setData] = useState<ProductionData>(emptyData);
