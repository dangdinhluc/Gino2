...
function formatPrice(priceCents: number, currency: string): string {
  ...
  return new Intl.NumberFormat('vi-VN', { style: 'currency', currency }).format(priceCents / 100);
}
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-5 px-4 py-5 pb-24 md:px-8">
      ...
      <section className="rounded-3xl border border-orange-200 bg-gradient-to-r from-[#fff9f3] to-[#ffeedd] p-6 md:p-8">
        ...
        <h1 className="mt-2 text-3xl font-black text-[#172033]">Chọn gói học Tokutei</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[#5f6b7c]">Gói miễn phí đăng ký ngay; thanh toán sẽ mở ở phase sau.</p>
      </section>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {packages.map((item) => (
            <article key={item.id} className="rounded-3xl border border-[#e8dccb] bg-[#fffaf3] p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-orange-50 text-orange-700"><Package size={22} /></span><span className="rounded-full bg-orange-50 px-3 py-1 text-sm font-black text-orange-700">{formatPrice(item.priceCents, item.currency)}</span></div>
              ...
              <button type="button" onClick={() => void handleEnroll(item)} disabled={item.priceCents !== 0 || busyId === item.id} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-orange-700 px-4 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:bg-slate-300">{busyId === item.id && <Loader2 size={16} className="animate-spin" />}{item.priceCents === 0 ? 'Đăng ký miễn phí' : 'Thanh toán sẽ mở sau'}</button>
            </article>
          ))}
        </div>
      )}
