...
export function VocabularyModeSegment({ mode, onModeChange, compact = false }: VocabularyModeSegmentProps) {
  const buttonClass = (active: boolean) => `flex h-full items-center justify-center gap-1 rounded-lg px-2 text-[10px] font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8d70dc] ${active ? 'bg-[#6f45d8] text-white' : 'text-[#858091] hover:bg-white'}`;
  return (
    <div className={`inline-flex shrink-0 items-center gap-0.5 rounded-[10px] border border-[#e8e3f2] bg-[#f8f7fc] p-0.5 ${compact ? 'h-11' : 'h-11 w-full'}`} role="tablist" aria-label="Chọn chế độ xem từ vựng">
