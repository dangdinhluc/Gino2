      ...
      {isOpen && (
        ...
        >
          <motion.section
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="learning-launcher-title"
            tabIndex={-1}
            ...
            className="relative flex max-h-[90dvh] min-h-0 w-full max-w-[500px] flex-col overflow-hidden rounded-t-[32px] border-t border-[#ebe3fa] bg-white shadow-[0_-16px_50px_rgba(25,15,50,0.2)] outline-none"
          >
            <div className="relative shrink-0 overflow-hidden bg-gradient-to-b from-[#eadefc] via-[#f4efff] to-white px-5 pb-3 pt-2.5">
              ...
              <button type="button" onClick={onClose} className="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full border border-white/80 bg-white/85 text-[#636573] shadow-xs backdrop-blur-xs transition-all hover:bg-white hover:text-[#1e1f26] active:scale-95" aria-label="Đóng Học ngay">
                <X size={17} strokeWidth={2.4} />
              </button>
              <img src={assets.shared.mascots.courseLauncherBanner} alt="一緒に勉強しよう!" className="mt-1 h-auto w-full rounded-[22px] object-cover drop-shadow-[0_4px_10px_rgba(111,69,216,0.12)]" />
              <div className="mt-1 text-center">
                <h2 id="learning-launcher-title" className="flex items-center justify-center gap-1.5 text-[22px] font-black tracking-tight text-[#1e1e24]"><span className="text-[#7144e8]">⚡</span><span>Học ngay</span></h2>
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain px-4 pb-[calc(6.5rem+env(safe-area-inset-bottom))] pt-3 touch-pan-y">
              {activeCourseStatus === 'error' && (
                <div className="rounded-[22px] border border-red-200 bg-red-50 p-4 text-center" role="alert">
                  ...
                  <button type="button" onClick={() => void retryActiveCourse()} className="mt-3 min-h-11 rounded-full bg-white px-4 text-[11px] font-black text-red-700 shadow-xs">Thử lại</button>
                </div>
              )}
              {activeCourseStatus !== 'ready' && activeCourseStatus !== 'error' && (
                <div className="space-y-3 rounded-[22px] border border-[#eae6f4] bg-white p-4" role="status"><div className="h-4 w-2/3 animate-pulse rounded-full bg-[#eeeaf8]" /><div className="h-3 w-full animate-pulse rounded-full bg-[#f4f1fb]" /><div className="h-12 w-full animate-pulse rounded-2xl bg-[#f4f1fb]" /></div>
              )}
              {activeCourseStatus === 'ready' && !activeCourseId && (
                <div className="rounded-[24px] border border-[#ded3f8] bg-[#f8f5ff] p-5 text-center">
                  ...
                  <button type="button" onClick={() => navigateTo('/app/courses')} className="mt-4 inline-flex min-h-11 items-center gap-1.5 rounded-full bg-[#6f45d8] px-5 text-[12px] font-black text-white shadow-[0_5px_14px_rgba(111,69,216,.22)] active:scale-[.98]">Chọn khóa học <ChevronRight size={15} /></button>
                </div>
              )}
              {activeCourseId && meta.isLoading && !activeCourseMeta && (
                <div className="space-y-3 rounded-[22px] border border-[#eae6f4] bg-white p-4" role="status"><div className="h-4 w-2/3 animate-pulse rounded-full bg-[#eeeaf8]" /><div className="h-3 w-1/3 animate-pulse rounded-full bg-[#f4f1fb]" /><div className="h-2 animate-pulse rounded-full bg-[#eeeaf8]" /></div>
              )}
              {activeCourseId && meta.loadError && !meta.isLoading && (
                <div className="rounded-[22px] border border-red-200 bg-red-50 p-4 text-center"><p className="text-[12px] font-bold text-red-700">{meta.loadError}</p><button type="button" onClick={meta.retry} className="mt-3 min-h-11 rounded-full bg-white px-4 text-[11px] font-black text-red-700 shadow-xs">Thử lại</button></div>
              )}
              {activeCourseMeta && (
                <>
