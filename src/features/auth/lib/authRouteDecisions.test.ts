...
import { decideAuthRouteAccess, getPublicLearnerEntryPath } from './authRouteDecisions';
...
assert.equal(getPublicLearnerEntryPath(false), '/login');
assert.equal(getPublicLearnerEntryPath(true), '/app/dashboard');
